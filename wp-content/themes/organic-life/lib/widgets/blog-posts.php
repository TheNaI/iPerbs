...
add_action('widgets_init','register_organic_blog_posts_widget');

function register_organic_blog_posts_widget()
{
	register_widget('organic_blog_posts_widget');
}

class organic_blog_posts_widget extends WP_Widget{

	function __construct()
	{
		parent::__construct( 'organic_blog_posts_widget','Organic blog Posts',array('description' => 'Organic post widget to display blog posts'));
	}
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order_by' ); ?>"><?php _e('Ordered By', 'themeum'); ?></label>
			<?php 
				$options = array(
					'popular' 	=> 'Popular',
					'latest' 	=> 'Latest', 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php _e('Count', 'themeum'); ?></label>
